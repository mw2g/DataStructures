...
public abstract class Document {
	...
	protected int countSyllables(String word) {
		...
		for (int i = 0; i < word.length(); i++) {
			...
			if ("eyuioaEYUIOA".indexOf(word.charAt(i)) != -1) {
				if (word.length() == 1) return 1;
				
				if ("qwrtpsdfghjklzxcvbnmQWRTPSDFGHJKLZXCVBNM".indexOf(word.charAt(i)) != -1) i++;
			} 
		}
		List<String> syllables = new ArrayList<>();
//		syllables = getTokens("[EUIOAYeuioay]+[QWRTPSDFGHJKLZXCVBNMqwrtpsdfghjklzxcvbnm]|[UIOAYuioay]\\s");
	...
	public double getFleschScore() {
		...
		double fleshScore = 206.835 - 1.015 * words / sentences - 84.6 * syllables / words; // TODO mw2g
