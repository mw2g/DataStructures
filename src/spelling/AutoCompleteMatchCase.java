...
//    	 prefix = prefix.toLowerCase();
    	 ...
    	 if (curr.endsWord()) {
    		 postfix.add(curr.getText());
    	 }
    	 queue.add(curr);
    	 
    	 while (!queue.isEmpty() && postfix.size() < numCompletions) {
    		 curr = queue.remove();
    		 Set<Character> set = curr.getValidNextCharacters();
    		 Object[] chas = set.toArray();
    		 
    		 for (Object ch : chas) {
    			 
    			 queue.add(curr.getChild((Character) ch));
    			 
    			 if (curr.getChild((Character) ch).endsWord()) {
    				 postfix.add(curr.getChild((Character) ch).getText());
    				 
    				 if (postfix.size() == numCompletions) {
    					 break;
    				 }
    			 }
    		 }
